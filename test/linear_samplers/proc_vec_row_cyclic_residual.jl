...
module ProceduralTestLSVRCyclicResidual

using Test, RLinearAlgebra

@testset "LSVR Cyclic Residual -- Procedural" begin

    # Verify appropriate super type
    @test supertype(LinSysVecRowResidCyclic) == LinSysVecRowSampler

    # Test whether the ordering is initialized correctly
    @test let
        cyc = LinSysVecRowResidCyclic()
        isempty(cyc.order)      
    end
