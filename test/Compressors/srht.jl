...
Random.seed!(1223)
@testset "SRHT" begin
    @testset "SRHT: Compressor" begin
        # Verify the supertype
        @test supertype(SRHT) == Compressor
        @test fieldnames(SRHT) == (
            :cardinality, :compression_dim, :block_size, :type          
        )
        @test fieldtypes(SRHT) == (Cardinality, Int64, Int64, Type{<:Number})
        # Verify the Internal Constructor
        let cardinality = Left(), comp_dim = 0, bs = 2, type = Float64
            @test_throws ArgumentError(
                "Field `compression_dim` must be positive."
            ) SRHT(
                cardinality, comp_dim, bs, type
            )
        end

        let cardinality = Left(), comp_dim = 1, bs = 0, type = Float64
            @test_throws ArgumentError(
        ...
        let cardinality = Undef(), comp_dim = 1, bs = 2, type = Float64
            @test_throws ArgumentError(
                "Cardinality must be of type `Left` or `Right`."
            ) SRHT(cardinality, comp_dim, bs, type)
        end

        # Verify external constructor and type 
        for Card in [Left, Right]
            compressor = SRHT(; cardinality=Card())
            typeof(compressor.cardinality) == Card
        end

        for type in [Bool, Int16, Int32, Int64, Float16, Float32, Float64]
            compressor = SRHT(cardinality=Right(), type=type)
            @test compressor.type == type
        end
    
    end

    @testset "SRHT: CompressorRecipe" begin
        @test_compressor SRHTRecipe
        #test supertype
        ...
        let card = Left(),
            ...
            recipe = SRHTRecipe(comp_dim, block_size, card, A, type)
            @test typeof(recipe) == SRHTRecipe{
                typeof(card), 
                Matrix{type}
            }
        ...
        # test the with non power of 2 row 
        let card = Left(),
            n_rows = 6,
            n_cols = 3,
            padded_dim = 8,
        ...
        # test the with non power of 2 col 
        let card = Right(),
            n_rows = 3,
            n_cols = 6,
            padded_dim = 8,
    ...
    @testset "SRHT: Complete Compressor" begin
        ...
        let card = Left(),
            ...
            compressor_recipe = complete_compressor(
                SRHT(; cardinality=card, compression_dim=c_dim, type=type), A
            )
    ...
    @testset "SRHT: Update Compressor" begin
        # test with left compressor
        let card = Left(),
            n_rows = 10,
            n_cols = 10,
    ...
    # Test multiplications with left compressors
    ...
    @testset "SRHT: Left Multiplication" begin
        ...
        let n_rows = 8,
            ...
            mul!(SA, S, A, 2.0, 1.0)
            tmp = (H * (ifelse.(S.signs, 1, -1) .* A) * 2 * S.scale)
            @test SA ≈ SAc + tmp[S.op, :] 
            mul!(Sy, S, A[:, 1], 2.0, 1.0)
            tmp_v = (H * (ifelse.(S.signs, 1, -1) .* A[:, 1]) * 2 * S.scale)
            @test Sy ≈ Syc + tmp_v[S.op] 
        end
        ...
        let n_rows = 10,
            ...
            mul!(SA, S, A, 2.0, 1.0)
            pad_mat[1:n_rows, :] = A
            pad_res = (H * (ifelse.(S.signs, 1, -1) .* pad_mat) * 2 * S.scale)
            @test SA ≈ SAc + pad_res[S.op, :]
            pad_vec[1:n_rows] = A[:, 1]
            pad_res = (H * (ifelse.(S.signs, 1, -1) .* pad_vec) * 2 * S.scale)
            mul!(Sy, S, A[:, 1], 2.0, 1.0)
            @test Sy ≈ Syc + pad_res[S.op]
        end
        ...
        let n_rows = 8,
            ...
            Sy = rand(n_rows)
            Syc = deepcopy(Sy)
            sI = zeros(n_rows, comp_dim)
            sI[S.op[1], 1] = 1
            sI[S.op[2], 2] = 1

            mul!(SA, A, S, 2.0, 1.0)
            tmp = (A * sI' * H) * Diagonal(ifelse.(S.signs, 1, -1)) * 2 * S.scale  
            testm = SAc + tmp
            @test SA ≈ testm
            mul!(Sy', A[1, :]', S, 2.0, 1.0)
            tmp_v = A[1, :]' * sI' * H * Diagonal(ifelse.(S.signs, 1, -1)) * 2 * S.scale
            testv = Syc' + tmp_v
            @test Sy ≈ testv'
        end
        ...
        let n_rows = 10,
            ...
            Sy = rand(size(S, 2))
            Syc = deepcopy(Sy)
            sI = zeros(pad_dim, comp_dim)
            sI[S.op[1], 1] = 1
            sI[S.op[2], 2] = 1
            
            # Test matrix multiplication
            mul!(SA, A, S, 2.0, 1.0)
            pad_res =  A * sI' * H * Diagonal(ifelse.(S.signs, 1, -1)) * S.scale
            @test SA ≈ SAc + 2 * pad_res[:, 1:size(A,1)]
            # Test vector multiplication
            mul!(Sy', A[1, :]', S, 2.0, 1.0)
            pad_res = A[1, :]' * sI' * H * Diagonal(ifelse.(S.signs, 1, -1)) * S.scale 
    ...
    @testset "SRHT: Right Multiplication" begin
        ...
        let n_rows = 2,
            ...
            mul!(SA, S, A, 2.0, 1.0)
            @test SA ≈ SAc + Diagonal(ifelse.(S.signs, 1, -1)) * H * sI * A * S.scale * 2
            mul!(Sy, S, A[:, 1], 2.0, 1.0)
            @test Sy ≈ Syc + (ifelse.(S.signs, 1, -1) .* H * sI * A[:,1]) * 2 * S.scale
        end
        ...
        let n_rows = 2;
            ...
            mul!(SA, S, A, 2.0, 1.0)
            pad_res = ifelse.(S.signs, 1, -1) .* H * sI * A * 2 * S.scale
            @test SA ≈ SAc + pad_res[1:size(S, 1), :]
            pad_res = ifelse.(S.signs, 1, -1) .* H * sI * A[:, 1] * 2 * S.scale
            mul!(Sy, S, A[:, 1], 2.0, 1.0)
        ...
        let n_cols = 8,
          ...
          mul!(SA, A, S, 2.0, 1.0)
          tmp = A * Diagonal(ifelse.(S.signs, 1, -1)) * H * 2 * S.scale
          testm = SAc + tmp[:, S.op] 
          @test SA ≈ testm
          mul!(Sy', A[1, :]', S, 2.0, 1.0)
          testv = A[1, :]' * Diagonal(ifelse.(S.signs, 1, -1)) * H * 2 * S.scale
          @test Sy ≈ Syc + testv[S.op]
      end
        ...
        let n_cols = 10,
            ...
            mul!(SA, A, S, 2.0, 1.0)
            pad_mat[:, 1:n_cols] = A
            pad_res =  pad_mat * Diagonal(ifelse.(S.signs, 1, -1)) * H * S.scale
            @test SA ≈ SAc + 2 * pad_res[:, S.op]
            # Test vector multiplication
            mul!(Sy', A[1, :]', S, 2.0, 1.0)
            pad_vec[1:n_cols] =  A[1, :]
            pad_res = pad_vec' * Diagonal(ifelse.(S.signs, 1, -1)) * H * S.scale
