...
Suppose we have a matrix ``A \\in \\mathbb{R}^{m \\times n}`` for which we wish to form a 
    ...
    orthogonal matrix, ``U``, a diagonal matrix, ``\\Sigma``, and an orthogonal matrix, 
    ``V`` such that ``U\\Sigma V^\\top \\approx A``. This can be done effectively using the 
    ...
    rank of the approximation. With this ``k``, RandomizedSVD  generates a compression 
    matrix ``S\\in\\mathbb{R}^{n \\times k}`` and computes ``Q = \\text{qr}(AS)`` as in the 
...
When the singular values decay slowly, we can improve the quality of the approximation using the 
    ...
    and take the qr factorization of ``(AA^\\top)^q AS``. Using these power iterations 
    increases the relative gap between the singular values leading to  better RandomizedSVD 
    performance. 
