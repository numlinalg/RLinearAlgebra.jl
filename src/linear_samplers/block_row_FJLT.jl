...
this procedure generates a matrix of the form S = G H D where G is sparse matrix with 
the non-zero entries being drawn from a gaussian distribution, H is a Hadamard matrix, 
...
"""
mutable struct LinSysBlkRowFJLT <: LinSysBlkRowSampler
    block_size::Int64
    sparsity::Float64 
    padded_size::Int64
...
LinSysBlkRowFJLT(;blocksize = 2, sparsity = .3) = LinSysBlkRowFJLT(
                                                   blocksize,
                                                   sparsity, 
                                                   0, 
                                                   nothing, 
