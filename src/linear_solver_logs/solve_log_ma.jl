...
function log_update!(
    ...
    if iter == 1 
        ...
        elseif typeof(sampler) <: LinSysBlkRowRandCyclic
            log.dist_info.block_dimension = maximum(size(sampler.blocks, 1))
    ...
    if iter > 0
        res = log.dist_info.scaling * (eltype(samp[1]) <: Int64 || size(samp[1],2) != 1 ? 
                       log.resid_norm(samp[3])^2 : log.resid_norm(dot(samp[1], x) - samp[2])^2) 
    else
        res::Float64 = norm(A * x - b)^2
    end
...
for type in (LinSysVecRowDetermCyclic,LinSysVecRowHopRandCyclic,
             ...
             LinSysVecRowRandCyclic, LinSysBlkRowReplace, LinSysBlkRowRandCyclic,
...
for type in (LinSysVecRowGaussSampler, LinSysVecRowSparseGaussSampler, LinSysBlkRowGaussSampler)
    @eval begin
        function get_SE_constants!(log::LSLogMA, sampler::Type{$type})
